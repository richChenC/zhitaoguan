...
try {
  ...
  await page.locator('[data-embedded-view="overview"]').click();
  await page.waitForTimeout(500);
  await page.screenshot({path: 'tmp/overview-model-redesign.png', fullPage: true});
  const canvasSize = await page.locator('#scene canvas').evaluate(canvas => ({width: canvas.width, height: canvas.height}));
  if (canvasSize.width < 500 || canvasSize.height < 400) throw new Error(`全景模型画布尺寸异常: ${JSON.stringify(canvasSize)}`);
