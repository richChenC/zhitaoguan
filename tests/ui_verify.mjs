...
if(!await indicationDialog.getByText('发现 2 条三字符缺失记录').isVisible()||!await indicationDialog.getByRole('button',{name:'保留原始值并标记待确认'}).isVisible())throw new Error('blank indication decision dialog is incomplete');
await page.screenshot({path:'tmp/browser/indication-import-warning.png',fullPage:true});
await indicationDialog.getByRole('button',{name:'取消导入，回原报告补全'}).click();
