...
await page.evaluate(()=>{window.resolveIndicationWarnings({blank_indication:{count:2,tubes:1,sources:1,items:[{outage:'F107',thimble_id:12,position:'F13',percent:51,location:'P1+42',filename:'DIR012.ECT'}]}});return true});
const indicationDialog=page.locator('.indication-warning-dialog');await indicationDialog.waitFor();
if(!await indicationDialog.getByText('发现 2 条三字符缺失记录').isVisible()||!await indicationDialog.getByRole('button',{name:'保留空值，待人工确认'}).isVisible()||!await indicationDialog.getByRole('button',{name:'一键将本批次缺失项设为 WAR'}).isVisible())throw new Error('blank indication decision dialog is incomplete');
await page.screenshot({path:'tmp/browser/indication-import-warning.png',fullPage:true});
await indicationDialog.getByRole('button',{name:'取消导入'}).click();
