(()=>{
  ...
  q('#runImport').onclick=async event=>{
    ...
    if(excelPath){q('#parseProgress').classList.add('active');status.textContent='正在校验 Excel...';try{let response=await fetch('/api/import-excel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:excelPath})}),data=await response.json();if(!response.ok)throw new Error(data.error||'Excel校验失败');if(data.requires_indication_decision){q('#parseProgress').classList.remove('active');const decision=await window.resolveIndicationWarnings(data);if(!['review','fill_war'].includes(decision)){status.textContent='已取消导入';return}q('#parseProgress').classList.add('active');status.textContent=decision==='fill_war'?'正在批量确认 WAR 并写入数据库...':'正在保留原始空值并写入数据库...';response=await fetch('/api/import-excel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:excelPath,blank_indication_policy:decision})});data=await response.json();if(!response.ok)throw new Error(data.error||'Excel写入失败')}status.textContent=`Excel入库完成：新增 ${data.inserted} 条，跳过重复 ${data.skipped} 条${data.blank_indication?.count?`，处理三字符 ${data.blank_indication.count} 条`:''}`;location.reload()}catch(error){status.textContent=error.message}finally{q('#parseProgress').classList.remove('active')}return}
    ...
    try{
      ...
      if(data.requires_indication_decision){q('#parseProgress').classList.remove('active');const decision=await window.resolveIndicationWarnings(data);if(!['review','fill_war'].includes(decision)){status.textContent='已取消导入';return}q('#parseProgress').classList.add('active');status.textContent=decision==='fill_war'?'正在批量确认 WAR 并写入数据库...':'正在保留原始空值并写入数据库...';const confirmed=await fetch('/api/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path,reports,blank_indication_policy:decision})});data=await confirmed.json();if(!confirmed.ok)throw new Error(data.error||'写入数据库失败')}
...
(function installDatabaseDeduplication(){
  function mount(){
    ...
    const panel=document.createElement('section');panel.id='databaseDedup';panel.className='panel software-settings-panel';panel.innerHTML=`<div class="panel-title"><div><h2>数据库去重</h2><span>只处理完全相同的检测记录，不合并不同入口、UID 或数据组</span></div><button id="scanDuplicates" type="button">扫描重复记录</button></div><div class="dedup-summary"><div><span>有效检测记录</span><strong id="dedupFindingCount">--</strong></div><div><span>累计拦截重复</span><strong id="dedupDuplicateCount">--</strong></div><div class="dedup-live-status"><i></i><span id="dedupStatus">正在读取去重状态</span></div><button id="runDeduplicate" class="primary" type="button">执行去重</button></div><div id="duplicateRecords" class="duplicate-records empty">正在读取去重信息...</div>`;host.append(panel);
    const records=panel.querySelector('#duplicateRecords');async function load(run=false){records.className='duplicate-records empty';records.textContent=run?'正在扫描并清理完全重复记录...':'正在读取去重信息...';try{const response=await fetch(run?'/api/deduplicate':'/api/duplicates',run?{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}:undefined),data=await response.json();if(!response.ok)throw new Error(data.error||'去重操作失败');panel.querySelector('#dedupFindingCount').textContent=data.findings;panel.querySelector('#dedupDuplicateCount').textContent=data.duplicates;records.className='duplicate-records';records.innerHTML=data.items.length?`<div class="duplicate-table-wrap"><table class="duplicate-table"><thead><tr><th>处理结果</th><th>重复记录</th><th>保留记录</th><th>检测时间</th><th>原因</th><th>来源文件</th><th>关键字段</th></tr></thead><tbody>${data.items.map(item=>{let payload={};try{payload=JSON.parse(item.payload_json||'{}')}catch{};return`<tr><td>${item.action==='removed'?'已清理':'已跳过'}</td><td>#${item.id}</td><td>#${item.kept_finding_id||'--'}</td><td>${item.detected_at||''}</td><td>${item.reason||''}</td><td title="${item.source_path||''}">${item.source_path||'--'}</td><td>${[payload.outage,payload.unit_id&&`机组${payload.unit_id}`,payload.thimble_id&&`通道${payload.thimble_id}`,payload.position,payload.indication||'未标注',payload.volts!=null&&`幅值 ${payload.volts}V`,payload.percent!=null&&`深度 ${payload.percent}%`].filter(Boolean).join(' · ')}</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty">未发现重复记录</div>';if(run)window.toast?.(`去重完成，累计处理 ${data.duplicates} 条重复记录`)}catch(error){records.textContent=error.message;window.toast?.(error.message)}}panel.querySelector('#scanDuplicates').onclick=()=>load(false);panel.querySelector('#runDeduplicate').onclick=()=>load(true);load(false);
  }
...
(function installDedupStatusFeedback(){const bind=()=>{const panel=document.querySelector('#databaseDedup'),records=panel?.querySelector('#duplicateRecords'),status=panel?.querySelector('#dedupStatus');if(!records||!status||records.dataset.statusBound)return;records.dataset.statusBound='1';new MutationObserver(()=>{if(records.classList.contains('empty')){status.textContent=records.textContent.startsWith('正在')?'正在扫描数据库':records.textContent;return}status.textContent=`状态已更新 · 累计拦截 ${panel.querySelector('#dedupDuplicateCount').textContent} 条 · ${new Date().toLocaleTimeString('zh-CN',{hour12:false})}`}).observe(records,{childList:true,subtree:true,characterData:true})};new MutationObserver(bind).observe(document.body,{childList:true,subtree:true});setTimeout(bind,200)})();
