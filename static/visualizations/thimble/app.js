...
function makePlateLabel(text,width,height,accent='#75866b',{fontSize=58,background='rgba(16,23,22,.88)',border=5}={}){const c=document.createElement('canvas');c.width=256;c.height=128;const x=c.getContext('2d');x.fillStyle=background;x.fillRect(2,2,252,124);x.strokeStyle=accent;x.lineWidth=border;x.strokeRect(5,5,246,118);x.fillStyle='#f4f7f5';x.font=`700 ${fontSize}px Consolas`;x.textAlign='center';x.textBaseline='middle';x.fillText(text,128,68);const texture=new THREE.CanvasTexture(c);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());const mesh=new THREE.Mesh(new THREE.PlaneGeometry(width,height),new THREE.MeshBasicMaterial({map:texture,transparent:true,side:THREE.DoubleSide,depthTest:false}));mesh.rotation.x=-Math.PI/2;mesh.renderOrder=20;return mesh}
...
const numberLabels=[];
function buildTubeNumbers(){numberGroup.clear();numberLabels.length=0;POSITIONS.forEach((position,index)=>{const{x,z}=coordinate(position),label=makePlateLabel(String(index+1),.68,.44,'#7f9189',{fontSize:64,border:4});label.position.set(x,1.14,z);label.userData.index=index;label.scale.setScalar(index===selected?1.12:1);label.material.color.set(index===selected?0xd7ef4a:0xffffff);numberGroup.add(label);numberLabels.push(label)})}
