...
function resize(){const w=host.clientWidth,h=host.clientHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()}new ResizeObserver(resize).observe(host);resize();
 ...
 function updateDefectTooltip(event){
   ...
   defectTooltip.innerHTML=`<b>${String(hit.object.userData.id).padStart(2,'0')}号管 · ${escapeHtml(hit.object.userData.position||'--')}</b><br>缺陷类型：${escapeHtml(row.indication||'--')} · 磨损深度：${Number.isFinite(percent)?percent+'%':'--'}<br>缺陷位置：${escapeHtml(row.location||'--')} · 测量通道：${escapeHtml(row.channel||'--')}<br>分析人员：${escapeHtml(row.analyst||'--')}`;
