...
const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(4,2,0);controls.enableDamping=true;controls.dampingFactor=.24;controls.rotateSpeed=.36;controls.panSpeed=.48;controls.zoomSpeed=.72;controls.minDistance=10;controls.maxDistance=85;
// 左键低速旋转，中键和右键平移，滚轮缩放。
...
function makePlateLabel(text,width,height,accent='#75866b',{fontSize=58,background='rgba(16,23,22,.88)',border=5}={}){const c=document.createElement('canvas');c.width=512;c.height=256;const x=c.getContext('2d');x.fillStyle=background;x.fillRect(4,4,504,248);x.strokeStyle=accent;x.lineWidth=border*2;x.strokeRect(10,10,492,236);x.fillStyle='#f4f7f5';x.font=`700 ${fontSize*2}px Consolas`;x.textAlign='center';x.textBaseline='middle';x.fillText(text,256,136);const texture=new THREE.CanvasTexture(c);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=renderer.capabilities.getMaxAnisotropy();const mesh=new THREE.Mesh(new THREE.PlaneGeometry(width,height),new THREE.MeshBasicMaterial({map:texture,transparent:true,alphaTest:.03,side:THREE.DoubleSide,depthTest:false}));mesh.rotation.x=-Math.PI/2;mesh.renderOrder=20;return mesh}

function buildCore(){
  const height=EMBEDDED?2.5:8.2,center=EMBEDDED?2.22:5.2;
...
const numberLabels=[];
function buildTubeNumbers(){const coreTop=EMBEDDED?3.5:9.32;numberGroup.clear();numberLabels.length=0;POSITIONS.forEach((position,index)=>{const{x,z}=coordinate(position),label=makePlateLabel(String(index+1),.68,.44,'#7f9189',{fontSize:64,border:4});label.position.set(x,coreTop,z);label.userData.index=index;label.scale.setScalar(index===selected?1.12:1);label.material.color.set(index===selected?0xd7ef4a:0xffffff);numberGroup.add(label);numberLabels.push(label)})}
function buildOrientation(){orientationGroup.clear();[['180°',0,-9.5],['0°',0,9.5],['90°',-9.5,0],['270°',9.5,0]].forEach(([text,x,z])=>{const marker=makePlateLabel(text,1.72,.58,'#d7ef4a',{fontSize:57,background:'rgba(8,13,12,.52)',border:4});marker.position.set(x,1.19,z);orientationGroup.add(marker)})}
...
function setSelected(index){selected=Math.max(0,Math.min(49,index));tubeGroups.forEach((g,i)=>{g.visible=currentCamera!=='tube'||i===selected;g.children.forEach(m=>{if(m.material===tubeMaterial||m.material===signalMaterial)m.material=i===selected?signalMaterial:tubeMaterial})});numberLabels.forEach((label,i)=>{label.scale.setScalar(i===selected?1.12:1);label.material.color.set(i===selected?0xd7ef4a:0xffffff)});const pos=POSITIONS[selected],{x,z}=coordinate(pos);detector.position.x=x;detector.position.z=z;detectorGlow.position.x=x;detectorGlow.position.z=z;document.querySelector('#tubeSelect').value=selected+1;document.querySelector('#tubeOutput').textContent=String(selected+1).padStart(2,'0');document.querySelector('#objectName').textContent=`指套管 #${String(selected+1).padStart(2,'0')}`;document.querySelector('#position').textContent=pos;buildExternalPath();updateDepth();updateArtificialDefect();if(currentCamera==='tube')setCamera('tube')}
...
const presets={overview:{p:EMBEDDED?[27,12,31]:[43,22,49],t:EMBEDDED?[3,-1.1,0]:[8,1,0]},section:{p:[32,7,35],t:[5,-1.8,0]},plate:{p:[0,36,.01],t:[0,1.1,0]},tube:{p:[28,2,22],t:[9,-2.2,0]}};
...
document.querySelectorAll('[data-visibility]').forEach(button=>button.onclick=()=>{const target=button.dataset.visibility;if(target==='shell'){shellVisible=!shellVisible;shellGroup.visible=currentCamera!=='tube'&&shellVisible}if(target==='core'){coreVisible=!coreVisible;coreGroup.visible=currentCamera!=='tube'&&coreVisible}if(target==='numbers'){numbersVisible=!numbersVisible;numberGroup.visible=currentCamera!=='tube'&&numbersVisible}if(target==='labels'){labelsVisible=!labelsVisible;labelsGroup.visible=currentCamera!=='tube'&&labelsVisible}button.classList.toggle('active',({shell:shellVisible,core:coreVisible,numbers:numbersVisible,labels:labelsVisible})[target])});
