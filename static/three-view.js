...
const P_LEVELS = {P1: 0.9, P2: 0.2, P3: -0.3, P4: -2.65, P5: -3.7, P6: -4.75};
const P_COLORS = {P1: 0xe45b4e, P2: 0x79857f, P3: 0x79857f, P4: 0xe45b4e, P5: 0x79857f, P6: 0x79857f};
...
function addThreeControls() {
  const toolbar = document.querySelector('.three-toolbar');
  if (!toolbar || toolbar.querySelector('.three-actions')) return;
  const actions = document.createElement('div'); actions.className = 'three-actions';
  actions.innerHTML = '<div class="three-camera-rail"><button type="button" data-three-camera="overview" class="active">总览</button><button type="button" data-three-camera="section">堆芯剖切</button><button type="button" data-three-camera="rack">分配架</button><button type="button" data-three-camera="tube">单管结构</button></div><button type="button" data-three-reset>重置视角</button><button type="button" data-three-defects>仅看缺陷</button><button type="button" data-three-scan>自动扫描</button>';
  toolbar.append(actions);
  actions.querySelector('[data-three-reset]').onclick = resetTopView;
  actions.querySelector('[data-three-defects]').onclick = event => { defectsOnly = !defectsOnly; event.currentTarget.classList.toggle('active', defectsOnly); load(activeScope); };
  actions.querySelector('[data-three-scan]').onclick=event=>{autoScan=!autoScan;event.currentTarget.classList.toggle('active',autoScan);if(autoScan){scanTimer=setInterval(()=>{camera.position.applyAxisAngle(new THREE.Vector3(0,1,0),.012);controls.update()},45)}else clearInterval(scanTimer)};
  actions.querySelectorAll('[data-three-camera]').forEach(button=>button.onclick=()=>setThreeCamera(button.dataset.threeCamera));
...
function addTubes(unit) {
  ...
  map.forEach((position, index) => {
    ...
    const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 13.25, 18), tubeMaterial.clone());
    tube.position.set(x, 2.575, z); tube.userData = {type:'tube',role:'coreTube',id:index+1,position}; model.add(tube); tubeMeshes.push(tube);
    const cap = new THREE.Mesh(new THREE.SphereGeometry(.14,18,10,0,Math.PI*2,0,Math.PI/2),tube.material);cap.position.set(x,9.2,z);cap.userData=tube.userData;model.add(cap);tubeMeshes.push(cap);
    const guide=new THREE.Mesh(new THREE.CylinderGeometry(.32,.32,4.7,20,1,true),guideMaterial());guide.position.set(x,-1.65,z);guide.userData=tube.userData;model.add(guide);
    const number = label(String(index + 1)); number.position.set(x, 9.5, z); number.userData = {type:'tube',role:'coreTube',id:index+1,position}; model.add(number); tubeMeshes.push(number);
  });
}
...
function addDefects(rows) {
  ...
  grouped.forEach(records => {
    ...
    const y = base + Math.max(-.22,Math.min(.22,offset/1200)); const {x, z} = coordinate(row.position); const color = defectColor(row.percent);
...
async function load(scope) {
  ...
  clearModel(); addEngineeringCore(); addEngineeringStructures(); addLayers(); if (unit) { addTubes(unit); addDefects((selectedRows.length ? selectedRows : activeRows).filter(row => !defectsOnly || defect(row))); addRackOverview(); } showStats(activeRows, unit, outage);setThreeCamera(threeCameraMode);
}
